...
| `/osaka` ショーケース | 長官デモの1枚目 | **未実装 → 本 ACT の主 Ask** |
...
| A4 | 全国自治体サーチは裏方継続でよいか | **Yes**（デモ主役にしない） |
...
| リスク | 緩和 |
|--------|------|
| 店名・KPI捏造 | ルール＋空 merchants＋レビュー |
| 肩書・組織名の誤り | 対外前に正式名称を FACT 化 |
| 二つのポータルで説明が割れる | `/about` を正本、ops は「工場」 |
| 探索が遅くデモが空 | 大阪を優先シード／手動で FACT な公開ソースのみ補充可 |
